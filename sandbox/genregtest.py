...
def main():
    ...
    parser.add_argument("--input", action="store", metavar="PLYFILE", help="Use PLY file as input in stead of synthetic pointcloud. Npoint is ignored")
    ...
    if args.single:
        offsets[3] = (0, 0, 0)
    if args.input:
        orig_pc = cwipc.cwipc_read(args.input, 0)
    else:
        ...
        gen = None
    assert orig_pc
    pc = create_regtest_pointcloud(orig_pc, offsets, args.two)
    cwipc.cwipc_write(output, pc)

def create_regtest_pointcloud(pc : cwipc.cwipc_wrapper, offsets : List[MyPoint], twocam : bool=False) -> cwipc.cwipc_wrapper:
